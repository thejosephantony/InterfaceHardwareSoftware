...
static int pqp_acelera_loop_00b4(PQP* cpu) {
     ...
     *   00e8 jmp 00b4
     *
     * A versão anterior só acelerava quando (r14-r13) era divisível por r13
     * sem overflow. Esta versão respeita registradores de 32 bits e resolve
     * o laço por congruência modular:
     *
    ...
    if(cpu->pcounter != 0x00B4 || !mem_tem_padrao(cpu, 0x00DC, padrao_tail, sizeof(padrao_tail))) {
        return 0;
    }
    ...
    /*
     * Todos os PCs de 00b4 até 00e4 executam na última volta também.
     * O jmp_00b4 em 00e8 executa uma vez a menos, pois na última volta
     * o JE é tomado e sai para 00ec.
     */
    ...
    /*
     * Efeito final conservador do tail. Em todas as versões observadas,
     * r15 é recalculado a partir de [0x5c] e deslocado antes da comparação.
     */
    cpu->registrador[14] = r13;

    if(mem_tem_padrao(cpu, 0x00D0, (const uint8_t[]){
        0x00, 0xF0, 0x5C, 0x00,
        0x02, 0xFF, 0x00, 0x00,
        0x0E, 0xF0, 0x00, 0x09
    }, 12)) {
