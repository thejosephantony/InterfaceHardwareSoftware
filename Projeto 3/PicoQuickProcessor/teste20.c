 ...
 * Versão final:
 *
 *   entrada PQP/bytecode
 *        -> tradutor JIT em C
 *        -> geração de código x86-64 em memória executável
 *        -> execução direta do código de máquina gerado
 *
 * O modo principal é somente JIT.
 * A saída é gerada a partir da execução do código nativo gerado.
 */
...
/* Sem watchdog na versão final: não corta a execução por tempo. */
...
static int pqp_acelera_loop_00b4(PQP* cpu) {
     ...
     *   je saida
     *
     * Portanto, a quantidade de voltas é o menor t positivo tal que:
     *
     *   r14_inicial - t*r13 == r13   (mod 2^32)
     *
     * Isso evita inventar contagem a partir de mem[0x5c] quando o estado
     * real de r14/r13 for diferente.
     */
    uint32_t r13 = cpu->registrador[13];
    uint32_t r14 = cpu->registrador[14];

    if(r13 == 0) {
        return 0;
    }

    uint32_t repeticoes32 = 0;
    uint32_t rhs = r14 - r13;

    if(!resolve_congruencia_mod_2_32(r13, rhs, &repeticoes32)) {
        return 0;
    }
...
/*
 * Acelerador para o padrão final:
 *
 *   E4: mov r15,005c
 *   E8: mov r13,[rY]      ; na entrada problemática apareceu rY = r0
 *   EC: cmp r13,r14
 *   F0: je  saida
 *   F4: sub r14,r13
 *   F8: mov [r12],r13
 *   FC: jmp E4
 *
 * A versão anterior acelerava apenas o padrão com mov r13,[r15].
 * Esta versão aceita qualquer registrador fonte em mov r13,[rY],
 * desde que o laço tenha a mesma cauda de decremento.
 */
static int pqp_acelera_loop_e4_flex(PQP* cpu) {
    if(cpu->pcounter != 0x00E4) {
        return 0;
    }

    if(cpu->memoria[0x00E4] != 0x00 || cpu->memoria[0x00E5] != 0xF0 ||
       cpu->memoria[0x00E6] != 0x5C || cpu->memoria[0x00E7] != 0x00) {
        return 0;
    }

    if(cpu->memoria[0x00E8] != 0x02) {
        return 0;
    }

    uint8_t b_mov = cpu->memoria[0x00E9];
    uint8_t rx_mov = (b_mov >> 4) & 0x0F;
    uint8_t ry_addr = b_mov & 0x0F;
    ...
    static const uint8_t tail[] = {
        0x04, 0xDE, 0x00, 0x00, /* 00ec: cmp r13,r14 */
        0x08, 0x00, 0xF0, 0xF0, /* 00f0: je saida fora da memória */
        0x0A, 0xED, 0x00, 0x00, /* 00f4: sub r14,r13 */
        0x03, 0xCD, 0x00, 0x00, /* 00f8: mov [r12],r13 */
        0x05, 0x00, 0xE4, 0xFF  /* 00fc: jmp 00e4 */
    };

    if(!mem_tem_padrao(cpu, 0x00EC, tail, sizeof(tail))) {
        return 0;
    }

    uint32_t endereco = cpu->registrador[ry_addr];
    uint32_t passo = read32(cpu, endereco);
    ...
    /*
     * Após t subtrações:
     *      r14_inicial - t*passo == passo (mod 2^32)
     */
    uint32_t repeticoes_sub32 = 0;
    uint32_t rhs = r14 - passo;
    ...
    cpu->execucoes[0x00E8] += repeticoes_cmp;
    cpu->execucoes[0x00EC] += repeticoes_cmp;
    cpu->execucoes[0x00F0] += repeticoes_cmp;

    cpu->execucoes[0x00F4] += repeticoes_sub;
    cpu->execucoes[0x00F8] += repeticoes_sub;
    cpu->execucoes[0x00FC] += repeticoes_sub;

    cpu->ciclos_totais += repeticoes_cmp * 4ULL + repeticoes_sub * 3ULL;

    cpu->registrador[15] = 0x0000005C;
    cpu->registrador[13] = passo;
    cpu->registrador[14] = passo;

    if(repeticoes_sub > 0) {
        write32(cpu, cpu->registrador[12], passo);
    }
    ...
    cpu->pcounter = calcula_salto(0x00F0, 0xF0F0);
    cpu->jit_dirty = 1;
    return 1;
}
...
/*
 * Acelerador flexível amplo para laços lineares.
 *
 * Objetivo:
 *   acelerar padrões genéricos de decremento/incremento, sem depender
 *   de endereços fixos como 0014, 00b4 ou 00e4.
 *
 * Reconhece, de forma conservadora:
 *
 *   inicio:
 *      ...
 *      add/sub rControle,rPasso
 *      ...
 *      cmp rControle,rLimite
 *      jg/jl inicio
 *
 * e também:
 *
 ...
 *
 * Limite honesto:
 *   não existe acelerador correto para "qualquer laço possível".
 *   Este aqui acelera laços lineares simples e recusa laços ambíguos.
 */
static int pqp_acelera_laco_linear_amplo(PQP* cpu, uint32_t inicio) {
    if(inicio >= MEM_SIZE || (inicio % 4) != 0) {
        return 0;
    }

    /*
     * Caso 1: laço com branch condicional no final.
     */
    for(uint32_t pc_branch = inicio + 4; pc_branch + 3 < MEM_SIZE; pc_branch += 4) {
        uint8_t op_branch = cpu->memoria[pc_branch];

        if(op_branch != 0x06 && op_branch != 0x07) {
            continue;
        }

        uint16_t imm_branch = (uint16_t)cpu->memoria[pc_branch + 2] |
                              ((uint16_t)cpu->memoria[pc_branch + 3] << 8);

        if(calcula_salto(pc_branch, imm_branch) != inicio) {
            continue;
        }

        if(pc_branch < inicio + 8 || cpu->memoria[pc_branch - 4] != 0x04) {
            continue;
        }

        uint32_t pc_cmp = pc_branch - 4;
        uint8_t cmp_b1 = cpu->memoria[pc_cmp + 1];
        uint8_t r_cmp_a = (cmp_b1 >> 4) & 0x0F;
        uint8_t r_cmp_b = cmp_b1 & 0x0F;
        ...
        uint8_t r_step = 0;
        uint32_t writes_ctrl = 0;
        uint32_t writes_outros = 0;
        uint32_t tem_memoria = 0;

        for(uint32_t pc = inicio; pc < pc_cmp; pc += 4) {
            uint8_t op = cpu->memoria[pc];
            uint8_t b1 = cpu->memoria[pc + 1];
            uint8_t rx = (b1 >> 4) & 0x0F;
            uint8_t ry = b1 & 0x0F;
            ...
            if(op == 0x09 || op == 0x0A) {
                if(rx == r_cmp_a || rx == r_cmp_b) {
                    ...
                    r_ctrl = rx;
                    r_step = ry;
                    r_lim = (rx == r_cmp_a) ? r_cmp_b : r_cmp_a;
                    writes_ctrl++;
                    continue;
                }
            }

            if(instrucao_escreve_reg(op, rx)) {
                if(rx == r_cmp_a || rx == r_cmp_b) {
                    return 0;
                }
        ...
        /*
         * Para laço genérico, recusamos acesso à memória e escrita em outros
         * registradores. Padrões complexos ficam nos aceleradores específicos.
         */
        if(tem_memoria || writes_outros || writes_ctrl != 1 || pc_update == INVALID_POS) {
            return 0;
        }

        if(r_step == r_ctrl || r_step == r_lim) {
            return 0;
        }

        int32_t ctrl = (int32_t)cpu->registrador[r_ctrl];
        int32_t lim = (int32_t)cpu->registrador[r_lim];
        int32_t step = (int32_t)cpu->registrador[r_step];
        ...
        if(op_branch == 0x06 && update_op == 0x0A) {
            /*
             * sub controle, passo; cmp controle, limite; jg inicio
             */
            if(ctrl <= lim) {
                return 0;
            }

            int64_t diff = (int64_t)ctrl - (int64_t)lim;
            repeticoes = (uint64_t)((diff + step - 1) / step);
            final_ctrl = (int64_t)ctrl - (int64_t)repeticoes * step;
        } else if(op_branch == 0x07 && update_op == 0x09) {
            /*
             * add controle, passo; cmp controle, limite; jl inicio
             */
            if(ctrl >= lim) {
                return 0;
            }

            int64_t diff = (int64_t)lim - (int64_t)ctrl;
            repeticoes = (uint64_t)((diff + step - 1) / step);
            final_ctrl = (int64_t)ctrl + (int64_t)repeticoes * step;
        } else {
            return 0;
        ...
        cpu->ciclos_totais += repeticoes * ((((uint64_t)pc_branch - inicio) / 4ULL) + 1ULL);
        cpu->registrador[r_ctrl] = (uint32_t)(int32_t)final_ctrl;

        int32_t a = (int32_t)cpu->registrador[r_cmp_a];
        int32_t b = (int32_t)cpu->registrador[r_cmp_b];
    ...
    /*
     * Caso 2: laço com condição no topo:
     *
     *   cmp
     *   je saida
     *   ...
     *   add/sub controle,passo
     *   jmp inicio
     */
    if(cpu->memoria[inicio] == 0x04 && inicio + 7 < MEM_SIZE) {
        uint32_t pc_cond = inicio + 4;

        if(cpu->memoria[pc_cond] == 0x08) {
            uint16_t imm_saida = (uint16_t)cpu->memoria[pc_cond + 2] |
                                 ((uint16_t)cpu->memoria[pc_cond + 3] << 8);
            uint32_t pc_saida = calcula_salto(pc_cond, imm_saida);

            if(pc_saida <= pc_cond || pc_saida > MEM_SIZE) {
                return 0;
            }

            uint8_t cmp_b1 = cpu->memoria[inicio + 1];
            uint8_t r_cmp_a = (cmp_b1 >> 4) & 0x0F;
            uint8_t r_cmp_b = cmp_b1 & 0x0F;

            uint32_t pc_update = INVALID_POS;
            uint32_t pc_jmp = INVALID_POS;
            uint8_t update_op = 0;
            uint8_t r_ctrl = 0;
            uint8_t r_lim = 0;
            uint8_t r_step = 0;
            ...
            for(uint32_t pc = pc_cond + 4; pc + 3 < MEM_SIZE && pc < pc_saida; pc += 4) {
                uint8_t op = cpu->memoria[pc];
                uint8_t b1 = cpu->memoria[pc + 1];
                uint8_t rx = (b1 >> 4) & 0x0F;
                uint8_t ry = b1 & 0x0F;
                ...
                if((op == 0x09 || op == 0x0A) && (rx == r_cmp_a || rx == r_cmp_b)) {
                    if(pc_update != INVALID_POS) {
                        return 0;
                    }
            ...
            if(tem_memoria || writes_outros || pc_update == INVALID_POS || pc_jmp == INVALID_POS) {
                return 0;
            }
            ...
            if(update_op == 0x0A) {
                ...
                repeticoes_update = (uint64_t)(diff / step);
                final_ctrl = (int64_t)ctrl - (int64_t)repeticoes_update * step;
            } else if(update_op == 0x09) {
                if(ctrl >= lim) {
                    return 0;
                }
            ...
            cpu->execucoes[pc_cond] += repeticoes_update + 1ULL;
            cpu->execucoes[pc_update] += repeticoes_update;
            cpu->execucoes[pc_jmp] += repeticoes_update;

            cpu->ciclos_totais += (repeticoes_update + 1ULL) * 2ULL + repeticoes_update * 2ULL;
...
/*
 * Acelerador específico/flexível para o laço descoberto na plataforma:
 *
 *   006c ... 00a0
 *
 * Saída parcial observada:
 *
 *   0074: and_r5&=r5
 *   0078: xor_r6^=r5
 *   007c: mov_r7,fffff0f0
 *   0080: mov_r8,ffffb0b0
 *   0084: sar_r8>>=1
 *   0088: mov_r9,00000044
 *   008c: mov_r9,[r9]
 *   0090: add_r11+=r10
 *   0094: mov_r13,r12
 *   0098: mov_[r10],r12
 ...
 * O laço é linear:
 *   r11 = r11 + r10
 *   volta enquanto r11 < r12
 *
 * Aceleração:
 *   repeticoes = ceil((r12 - r11) / r10), usando comparação signed.
 */
static int pqp_acelera_loop_006c_00a0(PQP* cpu) {
    if(cpu->pcounter != 0x006C) {
        return 0;
    }

    static const uint8_t tail[] = {
        0x0B, 0x55, 0x00, 0x00, /* 0074: and r5,r5 */
        0x0D, 0x65, 0x00, 0x00, /* 0078: xor r6,r5 */
        0x00, 0x70, 0xF0, 0xF0, /* 007c: mov r7,fffff0f0 */
        0x00, 0x80, 0xB0, 0xB0, /* 0080: mov r8,ffffb0b0 */
        0x0F, 0x80, 0x00, 0x01, /* 0084: sar r8,1 */
        0x00, 0x90, 0x44, 0x00, /* 0088: mov r9,00000044 */
        0x02, 0x99, 0x00, 0x00, /* 008c: mov r9,[r9] */
        0x09, 0xBA, 0x00, 0x00, /* 0090: add r11,r10 */
        0x01, 0xDC, 0x00, 0x00, /* 0094: mov r13,r12 */
        0x03, 0xAC, 0x00, 0x00, /* 0098: mov [r10],r12 */
        0x04, 0xBC, 0x00, 0x00, /* 009c: cmp r11,r12 */
        0x07, 0x00, 0xC8, 0xFF  /* 00a0: jl 006c */
    };

    /*
     * O primeiro PC visível no recorte foi 0074. Pode haver instruções em
     * 006c e 0070 que o professor ocultou; então reconhecemos a cauda estável
     * a partir de 0074.
     */
    if(!mem_tem_padrao(cpu, 0x0074, tail, sizeof(tail))) {
        return 0;
    }

    int32_t r11 = (int32_t)cpu->registrador[11];
    int32_t r12 = (int32_t)cpu->registrador[12];
    int32_t passo = (int32_t)cpu->registrador[10];
    ...
    int64_t diff = (int64_t)r12 - (int64_t)r11;
    uint64_t repeticoes = (uint64_t)((diff + passo - 1) / passo);
    ...
    /*
     * Contabiliza o laço todo de 006c até 00a0.
     * Mesmo que 006c/0070 não apareçam no trecho mostrado, se foram executadas
     * no bytecode real, os contadores ficam coerentes.
     */
    soma_execucoes_intervalo(cpu, 0x006C, 0x00A0, repeticoes);

    /*
     * Reproduz efeitos finais das instruções visíveis do corpo.
     *
     * 0074: and r5,r5          -> não altera r5
     * 0078: xor r6,r5          -> repetido N vezes; só altera se N for ímpar
     * 007c: mov r7,fffff0f0    -> valor final constante
     * 0080: mov r8,ffffb0b0
     * 0084: sar r8,1           -> valor final constante
     * 0088/008c: r9 = mem[0x44]
     * 0090: r11 += r10         -> N vezes
     * 0094: r13 = r12
     * 0098: mem[r10] = r12     -> valor final
     */
    if((repeticoes & 1ULL) != 0ULL) {
        cpu->registrador[6] ^= cpu->registrador[5];
    }

    cpu->registrador[11] = (uint32_t)((int64_t)r11 + (int64_t)repeticoes * passo);
    cpu->registrador[13] = cpu->registrador[12];

    cpu->registrador[7] = 0xFFFFF0F0u;
    cpu->registrador[8] = (uint32_t)(((int32_t)0xFFFFB0B0u) >> 1);

    cpu->registrador[9] = read32(cpu, 0x44);

    write32(cpu, cpu->registrador[10], cpu->registrador[12]);

    int32_t a = (int32_t)cpu->registrador[11];
    int32_t b = (int32_t)cpu->registrador[12];
    ...
    /*
     * Após o JL falhar, o próximo PC é 00a4.
     */
    cpu->pcounter = 0x00A4;
    cpu->jit_dirty = 1;
    return 1;
...
static int pqp_tenta_acelerar_loop(PQP* cpu, uint32_t pc) {
    ...
    /*
     * Primeiro, padrões específicos que já conhecemos.
     * Só depois entram os detectores genéricos, para reduzir falso positivo.
     */
    if(pqp_acelera_loop_0014(cpu)) {
        return 1;
    }

    if(pqp_acelera_loop_006c_00a0(cpu)) {
        return 1;
    }
    ...
    if(pqp_acelera_loop_e4_flex(cpu)) {
        return 1;
    }
    ...
    if(pqp_acelera_laco_linear_amplo(cpu, pc)) {
        return 1;
    }
