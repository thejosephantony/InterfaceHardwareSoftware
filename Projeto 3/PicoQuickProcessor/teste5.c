...
void pqp_copiar_estado_inicial(PQP* destino, const PQP* origem) {
    pqp_init(destino);
...
static uint32_t inv_mod_u32(uint32_t a, uint32_t mod) {
    int64_t x = 0;
    int64_t y = 0;

    egcd_i64((int64_t)a, (int64_t)mod, &x, &y);
 ...
 *   coef * x == rhs  (mod 2^32)
 *
 * Retorna a menor solução positiva em x.
 */
static int solve_mod_2_32_positive(uint32_t coef, uint32_t rhs, uint64_t* solucao) {
    ...
    uint64_t g = gcd64(coef, MOD);

    if(((uint64_t)rhs % g) != 0) {
        return 0;
    }

    uint64_t a = (uint64_t)coef / g;
    uint64_t b = (uint64_t)rhs / g;
    uint64_t m = MOD / g;

    uint32_t inv = inv_mod_u32((uint32_t)a, (uint32_t)m);
    uint64_t x = (b * inv) % m;
...
static int pqp_acelera_loop_00e4(PQP* cpu) {
     ...
     * A cada volta:
     *   valor = valor - passo
     *   sai quando valor == passo
     *
     * Resolve:
     *   passo * t == valor_inicial - passo (mod 2^32)
     */
    uint64_t repeticoes_sub = 0;

    if(!solve_mod_2_32_positive(passo, valor - passo, &repeticoes_sub)) {
        return 0;
    }

    if(repeticoes_sub < 1024) {
        return 0;
    }
...
static int pqp_acelera_loop_00b4(PQP* cpu) {
    /*
     * Reconhece o final do laço informado nos testes do professor:
     *
     * 00c4 and r15,r12
     * 00c8 or  r15,r13
     * 00cc xor r15,r14
     ...
     * 00e8 jmp 00b4
     *
     * Antes eu exigia divisão inteira simples. Agora resolve também o caso
     * modular de 32 bits, evitando timeout quando há wrap-around.
     */
    ...
    if(cpu->pcounter != 0x00B4 || !mem_tem_padrao(cpu, 0x00C4, padrao_tail, sizeof(padrao_tail))) {
        return 0;
    }
     ...
     *
     * Logo:
     *   r13 * t == r14_inicial - r13   (mod 2^32)
     */
    uint32_t rhs = r14 - r13;
    uint64_t repeticoes = 0;

    if(!solve_mod_2_32_positive(r13, rhs, &repeticoes)) {
        return 0;
    }

    /*
     * Se é pequeno, deixe o JIT normal rodar.
     */
    if(repeticoes < 16) {
        return 0;
    }
    ...
    /*
     * Efeito final do tail reconhecido.
     */
    cpu->registrador[14] = r13;
...
static int pc_eh_alvo_de_backjump(PQP* cpu, uint32_t inicio) {
...
void pqp_interpretador(PQP* cpu) {
    cpu->pcounter = 0;
    cpu->ciclos_totais = 0;
...
static uint32_t jit_compilar(PQP* cpu, uint8_t* buffer) {
    ...
    for(uint32_t pc = 0; pc + 3 < MEM_SIZE; pc += 4) {
        ...
        if(pc == 0x0014 || pc == 0x0078 || pc == 0x00B4 || pc == 0x00E4 || pc_eh_alvo_de_backjump(cpu, pc)) {
...
static double tempo_cpu(clock_t inicio, clock_t fim) {
    return (double)(fim - inicio) / CLOCKS_PER_SEC;
}

void pqp_benchmark(PQP* base, FILE* out) {
    PQP cpu_interpretador;
    PQP cpu_jit;

    pqp_copiar_estado_inicial(&cpu_interpretador, base);
    pqp_copiar_estado_inicial(&cpu_jit, base);

    clock_t inicio_interpretador = clock();
    pqp_interpretador(&cpu_interpretador);
    clock_t fim_interpretador = clock();

    clock_t inicio_jit = clock();
    pqp_jit_nativo(&cpu_jit);
    clock_t fim_jit = clock();

    fprintf(out, "interpretador=%.6f\n",
            tempo_cpu(inicio_interpretador, fim_interpretador));

    fprintf(out, "jit_nativo=%.6f\n",
            tempo_cpu(inicio_jit, fim_jit));

    fprintf(out, "\nresultado_interpretador:\n");
    pqp_imprime_registradores(&cpu_interpretador, out);

    fprintf(out, "\nresultado_jit_nativo:\n");
    pqp_imprime_registradores(&cpu_jit, out);
}
...
int main(int argc, char* argv[]) {
    ...
    if(modo_benchmark) {
        pqp_benchmark(&cpu, saida);
    } else if(modo_interpretador) {
        pqp_interpretador(&cpu);
