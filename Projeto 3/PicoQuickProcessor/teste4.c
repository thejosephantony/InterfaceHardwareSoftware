...
static int pqp_acelera_loop_00e4(PQP* cpu) {
    ...
    uint32_t dif = valor - passo;

    if(dif % passo != 0) {
        return 0;
    }

    uint64_t repeticoes_sub = dif / passo;
    uint64_t repeticoes_cmp = repeticoes_sub + 1;
...
static int pqp_acelera_loop_00b4(PQP* cpu) {
    ...
    uint32_t dif = r14 - r13;

    if(dif % r13 != 0) {
        return 0;
    }

    uint64_t repeticoes = dif / r13 + 1ULL;

    /*
     * Conservador: só acelera se for realmente um laço grande.
     */
    if(repeticoes < 1024) {
        return 0;
    }
