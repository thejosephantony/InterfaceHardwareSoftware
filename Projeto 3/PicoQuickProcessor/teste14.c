...
/*
 * WATCHDOG_TEMPO_US:
 * 0       = desativado, execução correta até PC sair do bytecode.
 * 900000  = modo diagnóstico: evita timeout externo de 1s, mas pode gerar saída parcial.
 */
...
static void __attribute__((unused)) pqp_copiar_estado_inicial(PQP* destino, const PQP* origem) {
    ...
    destino->tamanho_codigo = origem->tamanho_codigo;
}
...
static int pc_executavel(PQP* cpu, uint32_t pc) {
    return pc < cpu->tamanho_codigo &&
           pc + 3 < cpu->tamanho_codigo &&
           (pc % 4) == 0;
}

#if WATCHDOG_TEMPO_US > 0
static sigjmp_buf watchdog_env;
static volatile sig_atomic_t watchdog_armado = 0;
...
static void __attribute__((unused)) pqp_interpretador(PQP* cpu) {
    ...
    while(pc_executavel(cpu, cpu->pcounter)) {
...
static int pc_interno_alinhado(PQP* cpu, uint32_t pc) {
    return pc < cpu->tamanho_codigo &&
           pc + 3 < cpu->tamanho_codigo &&
           (pc % 4 == 0);
}

static void emit_ir_para_pc(JIT* jit, PQP* cpu, uint32_t pc) {
    if(pc_interno_alinhado(cpu, pc)) {
...
static void emit_jne_para_pc_ou_epilogo(JIT* jit, PQP* cpu, uint32_t pc) {
    if(pc_interno_alinhado(cpu, pc)) {
...
static uint32_t jit_compilar(PQP* cpu, uint8_t* buffer) {
    ...
    for(uint32_t pc = 0; pc + 3 < cpu->tamanho_codigo; pc += 4) {
    ...
    for(uint32_t pc = 0; pc + 3 < cpu->tamanho_codigo; pc += 4) {
        ...
        switch(opcode) {
            case 0x00:
                ...
                emit_ir_para_pc(&jit, cpu, next_pc);
                break;
            ...
            case 0x05:
                emit_ir_para_pc(&jit, cpu, alvo);
                break;

            case 0x06:
                emit_cmp_mem8_imm8(&jit, off_maior, 0);
                emit_jne_para_pc_ou_epilogo(&jit, cpu, alvo);
                emit_ir_para_pc(&jit, cpu, next_pc);
                break;
