...
static int pqp_acelera_loop_006c_00a0(PQP* cpu) {
    ...
    /*
     * Reconhecemos a cauda estável a partir de 0074.
     * As instruções 006c e 0070 também pertencem ao laço, mas não devem
     * ser ignoradas: elas podem inicializar r4, r5, r6 etc.
     */
    ...
    /*
     * Não acelera se 006c ou 0070 forem saltos. O acelerador só é seguro
     * quando o corpo 006c..009c é sequencial.
     */
    for(uint32_t pc = 0x006C; pc < 0x00A0; pc += 4) {
        uint8_t op = cpu->memoria[pc];
    ...
    int32_t r11_inicial = (int32_t)cpu->registrador[11];
    int32_t r12_limite = (int32_t)cpu->registrador[12];
    ...
    if(r11_inicial >= r12_limite) {
        return 0;
    }

    int64_t diff = (int64_t)r12_limite - (int64_t)r11_inicial;
    ...
    /*
     * Contabiliza todas as instruções do corpo e o JL.
     */
    soma_execucoes_intervalo(cpu, 0x006C, 0x00A0, repeticoes);

    /*
     * Em vez de "inventar" valores finais para r4/r5/r6, executamos
     * a ÚLTIMA iteração do corpo 006c..009c.
     *
     * Para isso, colocamos r11 no valor que ele tinha antes da última
     * iteração. Assim, o ADD de 0090 ainda é executado normalmente.
     *
     * Isso corrige o problema observado: r4/r5/r6 ficavam zerados porque
     * as instruções ocultas em 006c/0070 eram puladas pelo acelerador.
     */
    cpu->registrador[11] =
        (uint32_t)((int64_t)r11_inicial + (int64_t)(repeticoes - 1ULL) * passo);

    for(uint32_t pc = 0x006C; pc <= 0x009C; pc += 4) {
        uint8_t opcode = cpu->memoria[pc];
        uint8_t b1 = cpu->memoria[pc + 1];
    ...
    /*
     * Depois da última iteração, o JL de 00a0 falha e o fluxo segue para 00a4.
     */
    cpu->pcounter = 0x00A4;
    cpu->jit_dirty = 1;
    cpu->ciclos_totais += repeticoes * 14ULL;
    return 1;
}
...
static int pqp_tenta_acelerar_loop(PQP* cpu, uint32_t pc) {
    ...
    /*
     * Aceleradores específicos primeiro.
     * Genéricos só no final para evitar falso positivo.
     */
