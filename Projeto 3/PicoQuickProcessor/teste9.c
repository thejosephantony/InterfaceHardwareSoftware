...
static void emit_inc_counter(uint8_t *code, uint32_t *index, uint8_t pc)
{
    // inc dword ptr [rsi + (pc - 128)]
    // rsi aponta para instruction_counts[32]; como pc eh multiplo de 4, pc-128 aponta para instruction_counts[pc/4]
    code[(*index)++] = 0xFF;
    code[(*index)++] = 0x46;
    code[(*index)++] = (uint8_t)((int)pc - 128);
}
...
static void print_formatted_output(FILE *output, const struct Machine_x86 *vm, uint16_t pos, uint32_t exit_pc)
{
    ...
    {
        uint32_t count = vm->instruction_counts[pc / 4];
        if (count == 0)
            continue;
    ...
    while (pc < pos)
    {
        if (vm.not_interpreted[pc / 4])
        {
            ...
            case 0x00: // mov rx, i16
            {
                ...
                // inc dword ptr [rsi]
                emit_inc_counter(vm.executable_code, &index, pc);
        ...
        uintptr_t result = func(&vm.registers[0], &vm.instruction_counts[32], &vm.memory[0], &vm.save_bool);
