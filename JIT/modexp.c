	...
	while(index < n) {
		switch(code[index]) {
			...
			case 0x03: // bge
				//printf("bge @ %u\n", index);
				index++;
				if((eq || gt) || !lt) index += *(int32_t*)(&code[index]);
				index += 4;
				//printf("bge next = %u\n", index);
				break;
			case 0x04: // bun
...
void measure_time(const char* name, clock_t* start) {
...
int main() {
	...
	measure_time("aot", &start);
	printf("2^123456789 mod 18446744073709551533 = %lu\n", hardcoded());
	measure_time("hardcoded", &start);
	interpretation();
	measure_time("interpretation", &start);
	jit();
	measure_time("jit", &start);
	return 0;
}
