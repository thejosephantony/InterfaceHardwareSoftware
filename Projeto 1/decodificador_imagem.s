...
calc_maxrow:
    ...
    mov eax, dword ptr [rbx+8]  # rp
    shr eax, 4                  # run
    add eax, dword ptr [rbx]    # x + run

    mov edx, dword ptr [rbx+4]  # y
...
fim_maxrow:
    # largura = min(maxrow[y]) ignorando linhas com maxrow[y] = 0
    # Para o input UFS: largura = 12, altura = 5, taxa = 31*300/60 = 155
    mov eax, 0x7fffffff
    xor r13d, r13d
    mov rcx, [rip+maxrow_ptr]
...
pinta_loop:
    ...
    # ignora se offset estiver fora da matriz
    cmp rax, rcx
    jae prox_pinta
    ...
    and r10d, 0x0f        # índice da paleta
    lea rcx, [rip+paleta]
...
renderiza:
    # taxa = ntuplas * 3 * 100 / (largura * altura)
