...
loop_imagens:
    ...
    mov eax, [rip+v_ntup]
    test eax, eax
    jz prox_img_sem_free

    # aloca buffer de tuplas: ntup * 12 bytes
    ...
    # PASSO 1: le todas as tuplas, calcula alt
le_tuplas:
    mov eax, [rip+v_ntup]
    cmp r13d, eax
    jge fim_leitura
...
fim_leitura:
    # PASSO 2: aloca maxrow[alt] zerado via calloc
    ...
    # PASSO 3: calcula maxrow[y] = max(x+run) por linha
    xor r13d, r13d
calc_maxrow:
    mov eax, [rip+v_ntup]
    cmp r13d, eax
...
fim_maxrow:
    # PASSO 4: larg = min(maxrow[y]) para todo y != 0
    mov eax, 0x7FFFFFFF
    xor r13d, r13d
    mov rcx, [rip+maxrow_ptr]
calc_larg:
...
salva_larg:
    ...
    # PASSO 5: aloca matriz = larg * alt bytes
